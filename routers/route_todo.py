...
async def get_single_todo(id:str, response:Response,request:Request):
    new_token,_ = auth.veryfy_update
...
async def update_todo(response:Response,request:Request,id:str, data:TodoBody,csrf_protect:CsrfProtect = Depends()):
    new_token = auth.verify_jwt(
...
async def delete_todo(response:Response,request:Request,id:str, data:TodoBody,csrf_protect:CsrfProtect = Depends()):
    new_token = auth.verify_jwt(
        request,csrf_protect,request.headers
    )
